...
const fixTotkens = (tokenlist: any[][]) => {
	// OKX token logo
...
export async function getTokenList() {
	...
	const tokensByChain = mapValues(
		...
		(val: any) => uniqBy(val, (token: IToken) => token.address.toLowerCase())
	);

	let tokensFiltered = mapValues(tokensByChain, (val: any[], key: string | number) => {
		return val
			.filter((token: { address: string; }) => typeof token.address === 'string' && !tokensToRemove[key]?.[token.address.toLowerCase()])
			.map((token: { address: string; }) => ({ ...token, address: token.address.toLowerCase() }));
	});
	...
	for (const chain in tokensFiltered) {
		tokensFiltered[chain].forEach((token: { address: any; }) => {
	...
	if (geckoList && geckoList.length > 0) {
		geckoList.forEach((geckoToken: { platforms: any; }) => {
	...
	const formatAndSortTokens = (tokens: any[], chain: string) => {
		return tokens
			.map((t: { symbol: string; address: any; logoURI: any; }) => {
				const geckoId =
					geckoList && geckoList.length > 0
						? geckoList.find((geckoCoin: { symbol: any; }) => geckoCoin.symbol === t.symbol?.toLowerCase())?.id ?? null
						: null;

				const volume24h = 0;
//					topTokensByVolume[chain]?.find((item: { [x: string]: { token: any; }; }) => item['_id']?.token === t.address)?.['volume24h'] ?? 0;
			...
			.sort((a: { volume24h: number; }, b: { address: any; volume24h: number; }) => (b.address === ethers.constants.AddressZero ? 1 : b.volume24h - a.volume24h));
	};
