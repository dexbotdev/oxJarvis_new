...
import ReactSelect from 'src/components/MultiSelect'; 
import Route from 'src/components/SwapRoute';
import { getAllChains, inifiniteApprovalAllowed, swap } from './router';
import { TokenInput } from './TokenInput';
import Loader from './Loader';
import { useTokenApprove } from './hooks';
import { useGetRoutes } from 'src/queries/useGetRoutes';
import { useGetPrice } from 'src/queries/useGetPrice';
import { useTokenBalances } from 'src/queries/useTokenBalances';
...
import { formatSuccessToast } from 'src/utils/formatSuccessToast';
import { useDebounce } from 'src/hooks/useDebounce';
import { useGetSavedTokens } from 'src/queries/useGetSavedTokens';
import { ConnectButton } from '@rainbow-me/rainbowkit';
import { useLocalStorage } from 'src/hooks/useLocalStorage';
import SwapConfirmation from './SwapConfirmation';
import { useBalance } from 'src/queries/useBalance';
import { useEstimateGas } from './hooks/useEstimateGas';
import { ConnectButtonX } from '../ConnectButton';
...
const SelectWrapper = styled.div`
	border: ${({ theme }) => (theme.mode === 'dark' ? '1px solid #28243D;' : '1px solid #28243D;')};
	border-radius: 16px;
	padding: 12px;
	display: flex;
	flex-direction: column;
...
export function AggregatorContainer({ tokenlist }) {
	...
	const onChainChange = (newChain) => {
		setRoute(null);
		router.push({ pathname: '/swap', query: { chain: newChain.value } }, undefined, { shallow: true }).then(() => {
	...
	useEffect(() => {
		const id = setTimeout(() => {
			if (customSlippage && !Number.isNaN(Number(customSlippage)) && slippage !== customSlippage) {
				router.push({ pathname: '/swap', query: { ...router.query, slippage: customSlippage } }, undefined, {
					shallow: true
				});
			}
		}, 300);
 				...
 				<Body showRoutes={finalSelectedFromToken && finalSelectedToToken ? true : false}>
							...
							<Box display="flex" flexDir="column" marginX="4px">
								...
								<Box display="flex" gap="6px" flexWrap="wrap" width="100%">
									<Button
										...
										onClick={() => {
											setCustomSlippage('');
											router.push({ pathname: '/swap', query: { ...router.query, slippage: '0.1' } }, undefined, {
												shallow: true
											});
										}}
									>
									...
									<Button
										...
										onClick={() => {
											...
											router.push({ pathname: '/swap', query: { ...router.query, slippage: '0.5' } }, undefined, {
												shallow: true
											});
										}}
									>
									...
									<Button
										...
										onClick={() => {
											...
											router.push({ pathname: '/swap', query: { ...router.query, slippage: '1' } }, undefined, {
												shallow: true
											});
										}}
									>
					...
					{hasPriceImapct && !isLoading ? (
						<Alert status="warning" borderRadius="0.375rem" py="8px"> 
							High price impact! More than {priceImpact.toFixed(2)}%.
						</Alert>
					) : null}
					...
					<SwapWrapper>
						{!isConnected ? (
							...
							<Button  size='small' variant='contained'  onClick={() => switchNetwork(selectedChain.id)}>
								Switch Network
							</Button>
						) : insufficientBalance ? (
